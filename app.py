...
* **Eğitmen Kadrosu:** Bilgisayar programcılığı bölümündeki öğretim üyeleri Gönüllülük Çalışmaları Dersine Cenk Paşa girmekte aynı zamanda İletişim dersine de giriyor.Atatürk ilkeleri ve inklap tarihi dersine Uğur yıldırım girmekte. inglizce dersine Gamze Yavaş Çelik Girmekte. Algoritma ve Programlama Temelleri dersine   Ali ERFİDAN girmekte. Ağ yönetimi ve Bilgi güvenliği dersine Emre Selman CANIAZ girmekte aynı zamanda Temel ve Digital Elektronik dersine de girmekte.  Türk Dili dersine Gülfiye Bulut girmekte. Ofis yazılımları dersine Aykut DURGUT girmekte  Matematik I dersine  Tuğba KÜÇÜKSEYHAN girmekte aynı zamanda bu dersler Bilgisayar Programcılığı 1.sınıfın gördüğü tüm derslerdir. tüm dersler müfredata uygun ilerlemektedir. akademisyenlerimizin hepsi  güleryüzlü neşeli işini seven öğrencilerini seven değer veren kişilerdir.
...
def handle_special_query(client, prompt, model_name, myo_kaynagi, messages):
    ...
    if "OZETLE" in classification_response:
        ...
        last_bot_response = ""
        # messages[-2] ile son bot cevabını daha güvenli çekiyoruz.
        # Bu, kullanıcının "özetle" isteğinden önceki mesajın botun cevabı olduğunu varsayar.
    ...
    # Duygu Analizi kısmı tamamen kaldırılmıştır.
...
# --- 3. API ENTEGRASYONU ve CLIENT BAŞLATMA ---

# İstemcinin (client) Session State'e kaydedilip kaydedilmediğini kontrol et
if "client" not in st.session_state:
    try:
        # İstemciyi sadece bir kez oluştur ve Session State'e kaydet
        st.session_state.client = genai.Client(api_key=MY_API_KEY) 
    except Exception as e:
        st.error(f"API istemcisini başlatırken kritik hata: {e}")
...
st.caption("📌 **Kullanım Amacı:** Bu Yapay Zeka Asistanı, sadece **Altınoluk MYO** ve **Bilgisayar Programcılığı Bölümü** hakkındaki verilere dayanarak cevap üretir. Ayrıca özetleme ve sesli geri bildirim özelliklerine sahiptir. Lütfen sadece okul ve bölümlerle ilgili sorular yöneltin.")
